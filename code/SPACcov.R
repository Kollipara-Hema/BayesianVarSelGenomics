...
Allmodels<-function(Sim){ 
  ...
  q <- 150   # number of nonzero coefficients
  ...
  alpha_list <- list(c(0.1, 0.3, 0.8), c(0.2, 0.4, 0.8), c(0.3, 0.5, 0.8), c(0.6, 0.7, 0.9),c(0.8,0.5,0.9),c(0.3,0.3,0.3))
  alpha<-alpha_list[[6]]
 ...
 # beta <- c(rep(beta_s, q), rep(0, p - q))
  ...
  MSPE3out<-sum((outX_scale%*%ssL1$beta[,ncol(ssL1$beta)]-outY_scale)^2)/(outn)
  out3<-list(runtime=runtime3,confusion_measure(beta,ssL1$beta[,ncol(ssL1$beta)]),SSE=SSE3,MSPE=MSPE3,MSPEout=MSPE3out)
  ...
  df<-cbind(unlist(out1),unlist(out2),unlist(out4),unlist(out5),unlist(out6),unlist(out7),
            unlist(out8),unlist(out9),unlist(out10),unlist(out11),unlist(out12),unlist(out13),unlist(out3))
  colnames(df)<-c("lasso","EL","SL","HS","HSP","SN","blasso","RHS","SUSIE","STEP","RF","RFPC","SLF")
  write.csv(df,paste("All_","_Sim_",Sim,"_p_",p,"_spars_",spars,".csv",sep=""))
  beta_estimates<-as.data.frame(cbind(c(beta),c(ssL1$beta[,1]),c(model1$BetaHat),c(hsp$mu.beta),c(colMeans(ssN1$beta)),c(colMeans(model_bl$beta)),c(fit$coefficients),c(susie_get_posterior_mean(res)),
                                      c(final_lasso_coef),c(final_rf_coef),c(final_rf_coef.pc),c(lasso_model_optimal$beta[,1]),c(eln_model_optimal$beta[,1]),c(ssL1$beta[,ncol(ssL1$beta)])))
  colnames(beta_estimates)<-c("T","SL","HS","HSP","SN","blasso","RHS","SUSIE","STEP","RF","RFPC","lasso","EL","SLF")
  write.csv(beta_estimates,paste("beta_estimates_",Sim,"_p_",p,"_spars_",spars,".csv",sep=""))
}
#}
